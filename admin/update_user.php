    ...
    if ($foto) {
        ...
        if (in_array($extensi, $extensi_diperbolehkan) === true) {
            if ($ukuran < 5000000000) {
                ...
                $query = mysqli_query(
                    $link,
                    "UPDATE tb_user SET username = '$username', password = '$pass', email = '$email', foto = '$nama', level = '$level' WHERE id = '$id'"
                );
        ...
        $query = mysqli_query($link, "UPDATE tb_user SET username = '$username', password = '$pass', email ='$email', level = '$level' WHERE id = '$id'");
        if (!$query) {
            $isi = "Gagal Menambahkan Data dengan kesalahan =  " . mysqli_errno($link) . " - " . mysqli_error($link);
            echo "<meta http-equiv='refresh' content='0; url=home.php?page=tambah_user.php?status=$isi'>";
        } else {
